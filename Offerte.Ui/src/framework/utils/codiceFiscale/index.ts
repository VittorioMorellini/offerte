...
export class CodiceFiscale {
    ...
    constructor(data: any) {
        ...
        if (typeof data === 'object') {
            ...
            this.birthplace = new Comune(cfData.birthplace, cfData.birthplaceProvincia, cfData.birthplaceCc)
            this.compute()
        } else {
            throw new Error('Comune constructor accept either a valid string or a plain object. Check the documentation')
        }
    }
    static getCheckCode(codiceFiscale: string) {
    ...
    // }
    static computeInverse(codiceFiscale: string) {
        return new CodiceFiscale(codiceFiscale).toJSON()
    }
    static reverse(codiceFiscale: string) {
        return new CodiceFiscale(codiceFiscale).toJSON()
    }
    static compute(obj: any) {
        return new CodiceFiscale(obj).toString()
    }
    static check(codiceFiscale: any) {
    ...
    static dateCode(day: any, month: any, year: any, gender: any) {
        year = `0${year}`
    ...
    static toNumberIfOmocodia(input: any) {
        if (isNaN(input)) {
